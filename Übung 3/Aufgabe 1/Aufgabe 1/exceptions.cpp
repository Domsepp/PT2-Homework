...
// transforms a string to a date. Throws a logic_error if year is *not* between
// 1893 and 2018
...
void parseLine(const std::string& line, size_t lineNum) {
    ...
    try {  // Umwandlung des Niederschlages und Abfangen der Exception (Das gleich wird nochmal für Temp gemacht)
        ...
        throw FormatException("wrong number format");  // custom-Exception die an die aufrufende Funktion "geworfen" wird
    }
    try {  // Muss extra abgefangen werden, da Werte separat in log-Datei stehen sollen
        ...
        throw FormatException("wrong number format");
    }
    log_file.close();

    // std::cout << "Temp: " << temperature << "  Rain: " << rainfall <<
    // std::endl;
    //  TODO 3.1b: parse a given line, check dates by calling stringToTime,
    //  check temperature/rainfall by calling std::stof. Catch all exceptions
    //  thrown by these methods. If there have been any exceptions, aggregate
    //  all necessary information into an instance of FormatException and throw
    //  that instance.
}
...
void checkData(std::string fileName) {
    ...
    try {
        ...
        while (getline(file, s, '\n')) {
            try {
                ...
                // std::cout << s << std::endl;
                linenumber++;
            } catch (FormatException& ex) {  // Fängt die Format Exception ab, die von parseLine aufgerufen wurde
                writeOutFormatException(ex);
                num_exceptions++;  // Zähler für Anzahl der Exceptions
            }
        }
        file.close();
    ...
    log_file.close();
    // TODO 3.1c: read each line + call parseLine function (catch
    // FormatException) + count valid + invalid
    //            lines
}

int main(int argc, char* argv[]) {
    if (argc != 2) {
        std::cout << "Invalid number of arguments - USAGE: exceptions [DATASET]"
                  << std::endl;
        return -1;
    }
