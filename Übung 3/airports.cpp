...
// Using a custom struct increases readability compared to using
// std::pair<float, float> or std::array<float, 2>
struct Point {
    float latitude = 0.0F;
    float longitude = 0.0F;
};

// Calculates the distance between two points on earth specified by
// longitude/latitude. Function taken and adapted from
...
float calculateDistanceBetween(const Point& p1, const Point& p2) {
    // Up to C++17, there is no constant for PI in the standard, even though
    // support POSIX-API can use M_PI. tarting with C++20 there is a constant in
    // the standard for it: std::numbers::pi
    ...
    auto aHarv = std::pow(std::sin(dLat / 2.0), 2.0) + std::cos(dlat1) * std::cos(dlat2) * std::pow(std::sin(dLong / 2.0), 2.0);
    auto cHarv = 2.0 * std::atan2(std::sqrt(aHarv), std::sqrt(1.0F - aHarv));
    // Earth's radius from wikipedia varies between 6,356.750 km and 6,378.135
    // km The IUGG value for the equatorial radius of the Earth is 6378.137 km
            ...
            try {
                switch (fieldNum) {
                    case 0:  // id
                        ...
                        airportInfo.insert(
                            std::make_pair(currentID, AirportInfo()));
                        break;
                    case 1:  // name
...
void importRoutesData(const std::string& path, std::map<int, AirportInfo>& airportInfo) {
    ...
    while (std::getline(file, line)) {
        ...
        if (sourceID != -1 && destID != -1 && stops != -1) {
            try {
                // operator[] doesn't throw in case airport doesn't exists,
                // therefore use at()
                airportInfo.at(sourceID).routes.emplace_back(destID, stops);
            } catch (const std::out_of_range&) {
                std::cerr << "Source airport with ID " << sourceID
                          << " not present in airport dataset!" << std::endl;
            }
        }
    }
}
...
void removeNonDirectFlights(std::map<int, AirportInfo>& airportInfo) {
    ...
    for(auto& single_ap : airportInfo) {       
        int num_of_routes = single_ap.second.routes.size();
        auto it = std::remove_if(single_ap.second.routes.begin(),single_ap.second.routes.end(),[](std::pair<int, int> x) { return (x.second > 0); });
        single_ap.second.routes.erase(it,single_ap.second.routes.end());
    }
}
...
void calculateDistancePerRoute(std::map<int, AirportInfo>& airportInfo) {
    ...
    for(auto & single_ap : airportInfo){
        for(auto & single_route : single_ap.second.routes){
            single_ap.second.routeLengths.push_back(calculateDistanceBetween(single_ap.second.pos,airportInfo.at(single_route.first).pos));
        }
    }
}
...
void calculateAverageRouteDistances(std::map<int, AirportInfo>& airportInfo) {
    ...
    for(auto & single_ap : airportInfo){
        if(!(single_ap.second.routeLengths.size()==0)){
            float sum = std::accumulate(single_ap.second.routeLengths.begin(),single_ap.second.routeLengths.end(),0,[](float acc, float length){return acc += length;});
            single_ap.second.averageRouteLength = (sum/single_ap.second.routeLengths.size());
        }
    }
}
...
void printResults(std::map<int, AirportInfo>& airportInfo) {
    for (auto& airport : airportInfo) {
        if (!airport.second.routeLengths.empty()) {
            std::cout << airport.second.name << " (" << airport.second.city
                      << ", " << airport.second.country
                      << "): " << airport.second.averageRouteLength << "km ("
                      << airport.second.routeLengths.size()
                      << " direct outgoing routes)" << std::endl;
        }
    }
}
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "not enough arguments - USAGE: airports [AIRPORT DATASET] "
                     "[ROUTES DATASET]"
                  << std::endl;
